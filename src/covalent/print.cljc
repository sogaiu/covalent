...
;; heavily based on clojure/core.clj and clojure/core_print.clj

;; now, how to monitor those two files upstream to track changes :)

;; XXX: better to include all of core_print.clj and comment out unused portions?

(ns covalent.print
  #?(:clj
     (:refer-clojure
      :exclude [print-method pr pr-str
                *print-dup* *print-meta* *print-readably*
                *print-length* *print-level*
                *verbose-defrecords* *print-namespace-maps* 
                print-simple
                StackTraceElement->vec Throwable->map]))
  #?(:clj
     (:import [java.io Writer])))
  
;; slightly modified pr-str so that vars are printed using #object[...]
;; to allow interpretation as tagged literals

#?(:clj
   (do

     ;; XXX: hacks to avoid changing too much code below
     ...
     ;; XXX: probably don't need this -- see matching item below
     ...
     ;; XXX: since this is dynamic, may be it can be temporarily modified
     ;;      for out purposes?
     (defn pr
     ...
     (def ^:dynamic
       ^{:doc "*print-level* controls how many levels deep the printer will
        ...
        limit. Otherwise, it must be bound to an integer indicating the maximum
        level to print. Each argument to print is at level 0; if an argument 
        is a collection, its items are at level 1; and so on. If an object is a
     ...
     (defn- print-sequential
       [^String begin, print-one, ^String sep, ^String end, sequence,
        ^Writer w]
       (binding [*print-level*
                 (and (not *print-dup*) *print-level* (dec *print-level*))]
     ...
     ;; XXX: the whole reason for this file
     ...
     (prefer-method print-method
       clojure.lang.ISeq java.util.Collection)
     ...
     (prefer-method print-method
       clojure.lang.IPersistentCollection java.util.RandomAccess)
     ...
     (prefer-method print-method
       clojure.lang.IRecord java.util.Map)

     (prefer-method print-method
       clojure.lang.IRecord clojure.lang.IPersistentMap)
     ...
     (defmethod print-method StackTraceElement [^StackTraceElement o ^Writer w]
       (print-method [(symbol (.getClassName o)) (symbol (.getMethodName o))
                      (.getFileName o) (.getLineNumber o)] w))

     (defn StackTraceElement->vec
       "Constructs a data representation for a StackTraceElement:
       [class method file line]"
       [^StackTraceElement o]
       [(symbol (.getClassName o)) (symbol (.getMethodName o))
        (.getFileName o) (.getLineNumber o)])
     ...
     ;; XXX: does removing the \n that were here allow removal of base64
     ;;      elsewhere?
     ...
     ;; XXX: probably don't need -- see matching item near beginning
